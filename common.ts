...
import { readableStreamToString, BambooHrError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://{companyDomain}.bamboohr.com/authorize.php"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BambooHrError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BambooHrError(e, e.response?.data)
                }
                if (innerError instanceof BambooHrError) {
                    // Got "BambooHrError" from the above try block
                    throw innerError;
                }
